...
As far as I know, most if not all Karel simulator implement their own code interpreter, so that students can step through instructions one at a time.  However, this is not the case for Reeborg's World.  The reason for this is that I wanted for anyone using the program to be able to use **any** valid Python or Javascript code, using a generic interpreter implemented by someone else.  Here is what I do.
...
```py
_watch_(system_default_vars, loc=locals(), gl=globals())
RUR.set_lineno_highlight([0])
def add_and_print(a, b):
    _watch_(system_default_vars, loc=locals(), gl=globals())
    RUR.set_lineno_highlight([1])
...
Prior to execution of this code, a copy of variables found in `locals()` is made and copied in `system_default_vars`; these are assumed never to appear in a user's program. Each call to `_watch_` potentially result in an extra frame being created; actually, the content of what would be generated by `_watch_` in a new frame is actually copied into the previous frame's content to minimize the amount of frames created.
